...
# ./gradlew clean ui_tests -Dbrowser=CHROME -Dversion=121.0 -DwindowSize=1920x1080 -DremoteUrl=http://localhost:4444/wd/hub
...
case "$TEST_TYPE" in 
  ...
  ;;
  *)
    echo "Error: unable to run unknown tasks. Only ui_tests and api_tests are possible" >&2
    exit 1
  ;;
esac
